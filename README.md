...
| [OpenAI](https://openai.com) | Web-search-grounded answers for live, fact-sensitive questions only |
...
The AI layer uses **two endpoints**, chosen per feature:

- `/api/ai` — **IBM Granite via watsonx.ai**, for features that reason over the match context without needing real-time lookups (Match Guide, Choose My Team, Matchday Guide).
- `/api/ai-search` — the **OpenAI Responses API with the `web_search_preview` tool**, for features that may need current facts (Ask What Just Happened, Decision Explainer, Momentum & Tactics).
