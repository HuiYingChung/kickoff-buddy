...
So those features run a **two-stage hybrid**: `/api/ai-search` (GPT-4o) gathers the facts, then `/api/ai` (Granite) turns them into the explanation. If the fact-checker is unavailable, Granite still answers using the football-data.org events alone. Choose My Team and Matchday Guide stay Granite-only, since they don't hinge on the live match state.
